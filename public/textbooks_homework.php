<?php
// textbooks_homework.php - Textbooks and homework management (admin)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        $msg = t('textbooks_homework.delete_success');
    } catch (Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if ($name === '') {
        $error = t('textbooks_homework.enter_name');
    } elseif (empty($course_id)) {
        $error = t('textbooks_homework.select_course');
    } else {
        try {
            ...
            if (in_array('course_id', $columns)) {
                $stmt = $pdo->prepare('UPDATE subjects SET name = ?, course_id = ?, is_active = ? WHERE id = ?');
                $stmt->execute([$name, $course_id ?: null, $is_active, $edit_id]);
            } else {
                $stmt = $pdo->prepare('UPDATE subjects SET name = ?, is_active = ? WHERE id = ?');
                $stmt->execute([$name, $is_active, $edit_id]);
            }
            $msg = t('textbooks_homework.update_success');
        } catch (Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create'])) {
    ...
    if ($name === '') {
        ...
        try {
            ...
            $columns = $pdo->query("DESCRIBE subjects")->fetchAll(PDO::FETCH_COLUMN);
            if (in_array('course_id', $columns)) {
                $stmt = $pdo->prepare('INSERT INTO subjects (name, course_id, is_active) VALUES (?, ?, ?)');
                $stmt->execute([$name, $course_id ?: null, $is_active]);
            } else {
                $stmt = $pdo->prepare('INSERT INTO subjects (name, is_active) VALUES (?, ?)');
                $stmt->execute([$name, $is_active]);
            }
            $msg = t('textbooks_homework.create_success');
        } catch (Throwable $e) {
            $error = t('textbooks_homework.create_failed') . '：' . $e->getMessage();
        }
    }
}
...
try {
    ...
    if (in_array('course_id', $columns)) {
        ...
        $subjects = $pdo->query('SELECT id, name, is_active, created_at FROM subjects ORDER BY created_at DESC')->fetchAll();
    }
} catch (Throwable $e) {
    $subjects = [];
    $error = $error ?: (t('textbooks_homework.query_failed') . '：' . $e->getMessage());
}
?>
<!doctype html>
...
<head>
  ...
  <title><?= t('textbooks_homework.title') ?> - ERPH</title>
  ...
  <style>
    ...
    [data-theme="dark"] .subjects-form:hover,
    [data-theme="dark"] .subjects-list:hover {
...
<body>
  <header class="header">
    <h1>ERPH System - <?= t('textbooks_homework.title') ?></h1>
    <div>
  ...
  <div class="admin-layout">
    ...
    <main>
      <div class="page-header">
        <h2><?= t('textbooks_homework.title') ?></h2>
      </div>
      ...
      <div class="subjects-form">
        <form method="POST">
          <div class="form-group">
            <label for="name"><?= t('textbooks_homework.textbook_name') ?>:</label>
            <input type="text" id="name" name="name" required>
          </div>
          <div class="form-group
            <label for="course_id"><?= t('textbooks_homework.related_course') ?>:</label>
            <select name="course_id" id="course_id" required>
              <option value=""><?= t('textbooks_homework.select_course') ?></option>
          ...
          <button type="submit" name="create" class="submit-btn"><?= t('textbooks_homework.create_item') ?></button>
        </form>
      </div>

      <div class="subjects-list">
        <?php if (empty($subjects)): ?>
          <div style="text-align:center;padding:40px;color:#666;"><?= t('textbooks_homework.no_data') ?></div>
        <?php else: ?>
          <table>
            <thead>
              <tr>
                <th>ID</th>
                <th><?= t('textbooks_homework.textbook_name') ?></th>
                <th><?= t('textbooks_homework.related_course') ?></th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><?= htmlspecialchars($subject['course_title'] ?? t('textbooks_homework.missing_course_relation')) ?></td>
                <td>
                  <span class="status-<?= $subject['is_active'] ? 'active' : 'inactive' ?>">
                    <?= $subject['is_active'] ? t('common.enabled') : t('common.disabled') ?>
                  </span>
                </td>
                <td><?= htmlspecialchars($subject['created_at']) ?></td>
                <td>
                  <div class="action-buttons">
                    <button class="edit-btn" onclick="editSubject(<?= $subject['id'] ?>, '<?= htmlspecialchars($subject['name']) ?>', <?= $subject['course_id'] ?? 'null' ?>, <?= $subject['is_active'] ?>)"><?= t('common.edit') ?></button>
                    <button class="delete-btn" onclick="deleteSubject(<?= $subject['id'] ?>)"><?= t('common.delete') ?></button>
                  </div>
                </td>
              </tr>
              <?php endforeach; ?>
  ...
  <div id="editModal" class="modal">
    <div class="modal-content">
      <div class="modal-header">
        <h3><?= t('common.edit') ?> <?= t('textbooks_homework.title') ?></h3>
      ...
      <div class="modal-body">
        <form method="POST" id="editForm" action="">
          ...
          <div class="form-group">
            <label for="edit_name"><?= t('textbooks_homework.textbook_name') ?>:</label>
            <input type="text" id="edit_name" name="name" required>
          </div>
          <div class="form-group">
            <label for="edit_course_id"><?= t('textbooks_homework.related_course') ?>:</label>
  ...
  <script>
    ...
    function editSubject(subjectId, name, courseId, isActive) {
      console.log('Edit textbook/homework:', { subjectId, name, courseId, isActive });
    ...
    function deleteSubject(subjectId) {
      if (confirm('<?= t('textbooks_homework.confirm_delete') ?><?= t('textbooks_homework.confirm_delete_suffix') ?>')) {
        // Create form to submit delete request
        const form = document.createElement('form');
        form.method = 'POST';
        form.innerHTML = '<input type="hidden" name="delete_id" value="' + subjectId + '">';
